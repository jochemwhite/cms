...
function resolveMetadataFiles(
  metadata: CollectionEntryMetadataRow,
  files: MetadataFile[],
): Pick<CollectionEntryMetadata, "og_image_file" | "twitter_image_file"> {
...
async function hydrateMetadata(metadata: CollectionEntryMetadataRow): Promise<CollectionEntryMetadata> {
...
export async function getCollectionEntryMetadata(entryId: string): Promise<CollectionEntryMetadata | null> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("cms_collection_entry_metadata")
    .select("*")
    .eq("entry_id", entryId)
    .maybeSingle();
...
export async function upsertCollectionEntryMetadata(
  entryId: string,
  data: CollectionEntryMetadataUpdate,
): Promise<CollectionEntryMetadata> {
  ...
  const payload: Database["public"]["Tables"]["cms_collection_entry_metadata"]["Update"] &
    Pick<Database["public"]["Tables"]["cms_collection_entry_metadata"]["Insert"], "entry_id"> = {
    entry_id: entryId,
  ...
  const query = existingMetadata
    ? supabase.from("cms_collection_entry_metadata").update(payload).eq("entry_id", entryId)
    : supabase.from("cms_collection_entry_metadata").insert(payload);
